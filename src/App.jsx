...
function Detail({ book, onBack, onEdit, onDelete }){
  const cover = book.catalogCoverUrl || book.personalCoverDataUrl || "";
  return (
    <div style={{display:"grid", gap:10}}>
      <div style={styles.row}><span style={styles.link} onClick={onBack}>← Indietro</span></div>
      <div style={{...styles.card, display:"grid", gap:10}}>
        {cover ? <img alt="" src={cover} style={{width:"100%", maxHeight:420, objectFit:"contain", borderRadius:12, border:"1px solid #eee"}}/> : null}
...
export default function App(){
  ...
  async function refresh(){
    ...
    const next = {};
    for(const it of b){
      next[it.id] = await blobToObjectURL(it.catalogCoverUrl || it.personalCoverDataUrl || "");
    }
    setCovers(next);
  }
          ...
          <div style={{display:"grid", gap:10}}>
            ...
            {visible.length ? (
              <div style={styles.list}>
                {visible.map(b=><BookCard key={b.id} b={b} cover={covers[b.id]} onOpen={(x)=>{ setSelected(x); setView("detail"); }}/>)}
              </div>
            ) : (
